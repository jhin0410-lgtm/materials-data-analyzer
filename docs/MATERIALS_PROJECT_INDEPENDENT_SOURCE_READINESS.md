# Materials Project Same-Source New-Cohort Readiness
...
It is a **DevelopmentDiagnostic identity inventory**, not predictive evidence and not an independent benchmark result.

Most importantly, new `material_id` values remain from the **same Materials Project source system and broad DFT provenance**. Excluding the original 838 IDs can establish material-ID disjointness from benchmark-v1, but it does **not** establish source/provenance independence and must not be described as external validation.
...
Before freezing any same-source confirmatory policy/learner pair, the project must establish whether genuinely new Materials Project identities are available at all. A distinct authoritative source would still be required for source-disjoint external validation.
...
All 838 original `material_id` values are excluded from the current Materials Project identity response. The gate then reports the remaining new IDs and their chemical-system inventory.

This exclusion establishes only **ID disjointness within Materials Project**. It does not prove independence of calculation methodology, database curation, reference energies, software stack, or other source-level provenance.
...
- `independent_source_readiness.json` — source/database/query bindings, overlap counts, and explicit same-source independence limits;
- `independent_candidate_identity.csv` — only new material identity fields, deterministically sorted by `material_id`.

The candidate CSV contains no target, model score, policy score, or ranking. The JSON explicitly reports `same_source_system=true`, `source_independence_established=false`, and `external_validation_ready=false`.
...
- confirmatory benchmark execution;
- a source-independence claim;
- an external-validation claim;
- predictive or materials-discovery claims.

No arbitrary candidate-count threshold is applied at this stage. Candidate and group counts are measured first. If a new same-source cohort exists, the next change must predeclare any within-Materials-Project confirmatory design—including finalist policies, learner, partition/grouping rules, endpoints, label budget, adequacy criteria, and failure interpretation—**before** those new target labels are used.

Source-disjoint external validation remains a separate requirement and would need a compatible authoritative dataset or source with independently assessed provenance and property definitions.
