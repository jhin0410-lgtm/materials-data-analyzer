...
def _finite_numeric(table: pd.DataFrame, column: str) -> pd.Series:
    ...
    if values.isna().any() or not values.map(
        ...
        raise ValueError(
            f"Integrated table contains invalid numeric values in {column}."
        )
...
def validate_integrated_table(table: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            f"Integrated table is missing required column(s): {', '.join(missing)}"
        )
...
def _condition_matrix(validated: pd.DataFrame) -> pd.DataFrame:
    conditions = (
    ...
    if conditions["case_id"].duplicated().any():
        raise ValueError("Each case_id must map to exactly one process condition.")

    duplicated_conditions = conditions.duplicated([POWER, SPEED], keep=False)
    if duplicated_conditions.any():
        conflicts = (
            conditions.loc[duplicated_conditions, ["case_id", POWER, SPEED]]
            .sort_values([POWER, SPEED, "case_id"])
            .to_dict("records")
        )
        raise ValueError(
            "Each physical process condition must map to exactly one case_id; "
            f"duplicate mapping(s): {conflicts}."
        )
...
def audit_process_design(table: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    grouped = _condition_matrix(validated)
    ...
    quadratic = np.column_stack([interaction, power_z**2, speed_z**2])
    designs = {
    ...
    direct_power_contrast_available = any(
        value >= 2 for value in power_overlap_by_speed.values()
    )
    ...
    audit = {
        ...
        "replication": {
            ...
            "all_conditions_replicated": bool(
                (grouped["replicate_count"] >= 2).all()
            ),
        },
        "factor_support": {
...
def run_audit(
    integrated_table_path: str | Path,
    output_dir: str | Path,
) -> dict[str, Path]:
    ...
    outputs = {
        "condition_matrix": condition_path,
        "audit": audit_path,
        "report": report_path,
    }
    manifest = {
