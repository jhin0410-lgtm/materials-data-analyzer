...
def test_router_structurally_intakes_unseen_csv_without_accepting_analysis(tmp_path):
    ...
    assert proposal["candidate_identifier_is_provenance_only"] is True
    assert proposal["semantic_proposal"]["candidate_identity_columns"] == [0]
    assert proposal["semantic_proposal"]["candidate_id_is_scientific_identity"] is False
    assert proposal["lineage_proposal"]["candidate_id_is_specimen_identity"] is False
    ...
    assert proposal["review_request_scope"] == ["scientific_intake"]
...
def test_router_uses_artifact_sha_as_non_scientific_candidate_id_when_missing(tmp_path):
    ...
    assert proposal["candidate_id"] == (
        "artifact-sha256:" + receipt["artifact_sha256"][:24]
    )
    assert proposal["candidate_identifier_is_provenance_only"] is True
