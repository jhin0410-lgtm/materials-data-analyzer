...
from materials_data_analyzer.research_loop import scientific_control_plane_impl
...
def test_execution_bearing_workstream_and_metadata_receive_exact_split_projections() -> None:
    ...
    assert (
        workstream["governance_projection"]["execution_adapter_selection"]
...
def test_internal_control_plane_validator_is_not_a_weaker_type_boundary() -> None:
    ...
    cycle = next(
        item
        for item in contract["controller_inventory"]
        if item["surface_id"] == "research_cycle"
    )
    cycle["maximum_actions_per_call"] = 1.0
    with pytest.raises(
        scientific_control_plane_impl.ScientificControlPlaneError,
        match="controller_inventory drifted",
    ):
        scientific_control_plane_impl.validate_scientific_control_plane_contract(contract)
