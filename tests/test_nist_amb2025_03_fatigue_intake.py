...
def _xlsx(rows: list[list[object]], *, formula_f3: bool = False) -> bytes:
    ...
    for row_number, values in enumerate(rows, start=1):
        ...
        for index, value in enumerate(values, start=1):
            ...
            formula = (
                "E3*2"
                if formula_f3 and row_number == 3 and column == "F"
                else None
            )
...
def _rows() -> list[list[object]]:
    return [
        ...
        [
            ...
            None,
            "Op",
            "invalid test, load application error, stopped test manually",
        ],
    ]
...
def test_runout_notes_override_cycles_column_event_semantics() -> None:
    ...
    inventory = report["fatigue_inventory"]
    assert inventory["observed_failures"] == 1
    assert inventory["runouts"] == 2
    assert inventory["invalid_tests"] == 1
    assert inventory["desired_load_formula_cell_count"] == 1
    assert inventory["diameter_source_storage_counts"] == {
        "numeric_cell": 3,
        "text_decimal_cell": 1,
    }
    assert report["runout_reconciliation"][
    ...
    assert report["analysis_eligibility"]["naive_uncensored_cycles_regression"][
        "eligible"
    ] is False
    assert report["analysis_eligibility"]["condition_specific_censored_sn_analysis"][
        "eligible"
    ] is False
    assert report["scientific_status_changed"] is False

    text_diameter = next(
        item for item in report["records"] if item["specimen_id_source"] == "18"
    )
    assert text_diameter["diameter_mm"] == 4.09
    assert text_diameter["diameter_source_storage"] == "text_decimal_cell"

    runout = next(
        item for item in report["records"] if item["specimen_id_source"] == "42"
    )
...
def test_observed_failure_requires_no_runout_or_invalid_note() -> None:
    ...
    failure = next(
        item for item in report["records"] if item["specimen_id_source"] == "114"
    )
    invalid = next(
        item for item in report["records"] if item["specimen_id_source"] == "54"
    )
...
def test_core_event_semantics_may_not_be_formula_derived() -> None:
    ...
    with zipfile.ZipFile(io.BytesIO(workbook), "r") as source, zipfile.ZipFile(
        ...
        for name in source.namelist():
            ...
            if name == "xl/worksheets/sheet1.xml":
                ...
                text = text.replace(
                    '<c r="G5"><v>28467</v></c>',
                    '<c r="G5"><f>10000+18467</f><v>28467</v></c>',
                )
    ...
    with pytest.raises(
        NistAmb202503FatigueIntakeError,
        match="core fatigue authority contains formula",
    ):
        audit_amb2025_03_fatigue(
            workbook_bytes=output.getvalue(), readme_bytes=README
        )


def test_non_decimal_diameter_text_fails_closed() -> None:
    rows = _rows()
    rows[2][3] = "4.09 mm"
    with pytest.raises(
        NistAmb202503FatigueIntakeError,
        match="numeric cell or exact decimal text",
    ):
        audit_amb2025_03_fatigue(
            workbook_bytes=_xlsx(rows), readme_bytes=README
        )
