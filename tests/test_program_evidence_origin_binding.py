...
def test_authenticates_exact_program_membership_and_origin_identity() -> None:
    ...
    assert result["supplied_program_state_membership_established"] is True
    assert result["exact_evidence_identity_joined"] is True
    assert result["program_state_trust_boundary"] == "caller_supplied_not_reauthenticated"
    assert result["program_state_provenance_reauthenticated"] is False
