...
def test_science_plane_can_initialize_scaffolding_before_evidence_ingestion() -> None:
    assert "science_plane_authored" in SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["research_question"]
    assert "science_plane_authored" in SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["scientific_mission"]
    assert "science_plane_authored" in SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["hypothesis"]
    assert "science_plane_planner" in SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["candidate_action"]
    assert "science_plane_decision" in SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["decision"]


def test_empirical_and_semantic_updates_still_require_validated_evidence_authority() -> None:
    assert SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["observation"] == "validated_evidence_packet"
    assert SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["evidence"] == "validated_evidence_packet"
    assert "validated_evidence_packet" in SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["derived_result"]
    assert "authority_bearing_epistemic_update" in SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["claim"]
    assert "validated_evidence_packets" in SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["claim"]
    assert "authority_bearing_epistemic_update" in SCIENTIFIC_ENTITY_AUTHORITY_SOURCES["inference"]
...
def test_each_run_section_has_explicit_authority_source() -> None:
    ...
    assert sources["scientific_state"] == (
        "entity_specific_science_plane_initialization_and_validated_evidence_epistemic_update_rules"
    )
    assert contract["scientific_entity_authority_sources"] == SCIENTIFIC_ENTITY_AUTHORITY_SOURCES
