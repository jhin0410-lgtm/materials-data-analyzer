...
def _after_state(action_type: str, report: Path) -> dict[str, object]:
    return {
        "actions": [
...
def test_explicit_authorized_action_dispatches_once_and_reverifies(
    ...
    report = root / "action_result.json"
...
def test_unknown_selected_action_fails_before_dispatch(
    ...
    with pytest.raises(execution.AuthorizedExecutionError, match="no hardcoded typed executor"):
...
def test_executor_must_append_exactly_one_action(
    ...
    monkeypatch.setitem(
        execution._DISPATCH,
        "protocol_stratification",
        (lambda path: {"ok": True}, lambda path: {"verified": True}),
    )
    after = _before_state()
    after["actions"] = [
