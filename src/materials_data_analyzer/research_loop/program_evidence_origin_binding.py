"""Bridge caller-supplied program evidence identity to exact evidence-origin provenance.

This module is deliberately narrow. It proves that one legacy program evidence binding
(`workstream_id`, `role`, `sha256`) is present in the supplied program state and
...
def authenticate_program_evidence_origin_binding(
    *,
    ...
    """Authenticate one caller-supplied program-evidence identity plus its origin record.

    The program state is a caller-supplied input trust boundary. This function does not
    authenticate how that state was produced; it proves only exact membership in the supplied
    state and exact SHA identity with the origin-classification evidence bytes.
    """
    ...
    matches = _program_evidence_matches(program_state, binding)
    if matches != 1:
        if matches == 0:
            raise ProgramEvidenceOriginBindingError(
    ...
    return {
        ...
        "supplied_program_state_membership_established": True,
        "program_state_trust_boundary": "caller_supplied_not_reauthenticated",
