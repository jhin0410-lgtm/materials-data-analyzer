"""Authorized typed execution for the audited reference heat-conduction action."""
...
from .heat_execution_verifier import ADAPTER_ID
...
def _build_result(
    *,
    ...
    return {
        "schema_version": "1.0",
        "execution_policy_version": "1.8+reference-heat-1.0",
        ...
        "scientific_boundary": (
            "This action can establish numerical reference-solver behavior and analytical numerical "
            "validation only. It cannot establish material-specific, process-specific, or empirical validity."
        ),
    }
...
def _finish_recovery(
    *,
    ...
    if recovery.get("action_type") != ACTION_TYPE or recovery.get("action_version") != ACTION_VERSION:
        raise AuthorizedExecutionError("recovered heat action type/version binding drifted")
    if recovery.get("cost_units") != COST_UNITS:
        raise AuthorizedExecutionError("recovered heat action cost binding drifted")
    action_id = recovery.get("action_id")
    if not isinstance(action_id, str) or request.get("action_id") != action_id:
        raise AuthorizedExecutionError("recovered heat action_id differs from pinned request")
    ...
    if not isinstance(state, Mapping) or not isinstance(state.get("actions"), list):
        raise AuthorizedExecutionError("recovered heat research state is malformed")
    ...
    if not isinstance(registry, Mapping):
        raise AuthorizedExecutionError("recovered heat execution registry is malformed")
...
def execute_heat_authorized_action(
    *,
    ...
    if request_record.get("sha256") != request_sha_pin:
        raise AuthorizedExecutionError(
            "exact heat execution request bytes differ from the independently verified request SHA-256"
        )
    ...
    with shared_research_ledger_transaction_lock(run):
        ...
        if before.get("ledger_sha256") != ledger_sha_pin:
            raise AuthorizedExecutionError(
                "research ledger changed after independent reference-heat verification"
            )
        ...
        if authorization.get("authorization_status") != "ready_for_explicit_execution_request":
            raise AuthorizedExecutionError(
                "reference heat action is not ready for explicit typed execution"
            )
        ...
        if not isinstance(selected, Mapping) or not isinstance(contract, Mapping):
            raise AuthorizedExecutionError("reference heat authorization omitted action contract")
        if (
            ...
            raise AuthorizedExecutionError("reference heat authorization action/version/cost drifted")
        ...
        if request.get("action_version") != ACTION_VERSION:
            raise AuthorizedExecutionError("execution request action_version differs from authorized heat action")
        before_actions = before.get("actions")
        if not isinstance(before_actions, list):
            raise AuthorizedExecutionError("pre-execution heat action ledger is malformed")
        ...
        if transaction.get("recovered"):
            ...
            if not isinstance(recovered_state, Mapping):
                raise AuthorizedExecutionError("prepared heat transaction recovery state is malformed")
            return _finish_recovery(
                request=request,
                request_file=request_file,
                request_record=request_record,
        ...
        result = execute_heat_conduction_action_preparsed(
            request,
            request_path=request_file,
            request_record=request_record,
        )
        ...
        if not isinstance(after_actions, list) or len(after_actions) != before_count + 1:
            raise AuthorizedExecutionError("heat typed action must append exactly one ledger action")
        if after_actions[-1].get("action_id") != action_id:
            raise AuthorizedExecutionError("heat typed action ledger action_id drifted")
        mark_action_output_ledger_committed(
            research_run=run,
            action_id=action_id,
            action_type=ACTION_TYPE,
...
__all__ = ["execute_heat_authorized_action"]
