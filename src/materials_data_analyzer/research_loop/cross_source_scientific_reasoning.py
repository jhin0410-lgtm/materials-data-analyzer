...
class UncertaintyComponent:
    ...
    def __post_init__(self) -> None:
        if self.category not in {"measurement", "model", "sampling", "extrapolation", "provenance"}:
        ...
        if not math.isfinite(self.standard_uncertainty) or self.standard_uncertainty < 0:
            raise CrossSourceReasoningError("standard uncertainty must be finite and non-negative")


def combine_uncertainty(
    components: tuple[UncertaintyComponent, ...], *, independence_explicitly_established: bool
) -> float:
    if not components:
        raise CrossSourceReasoningError("at least one uncertainty component is required")
...
class EffectEstimate:
    ...
    def __post_init__(self) -> None:
        ...
        if not math.isfinite(self.standard_uncertainty) or self.standard_uncertainty < 0:
            raise CrossSourceReasoningError("effect uncertainty must be finite and non-negative")
...
def detect_verified_directional_contradiction(
    ...
    *,
    ...
    if interval_multiplier <= 0 or not math.isfinite(interval_multiplier):
        raise CrossSourceReasoningError("interval_multiplier must be finite and positive")
    ...
    contradiction = (left_low > 0 and right_high < 0) or (left_high < 0 and right_low > 0)
...
def select_next_analysis(traits: AnalysisTraits) -> AnalysisRecommendation:
    ...
    if min(traits.n_samples, traits.n_numeric_predictors, traits.group_count, traits.repeated_measure_groups) < 0:
        raise CrossSourceReasoningError("analysis trait counts must be non-negative")
    if traits.target_kind not in {"continuous", "categorical", "none"}:
    ...
    if traits.target_kind == "continuous" and traits.n_numeric_predictors >= 1 and traits.n_samples >= 8:
        return AnalysisRecommendation(
            "existing_data_reanalysis", "bounded_regression", True,
            ("continuous_target", "numeric_predictor_available", "minimum_sample_contract_met"),
        )
    if traits.target_kind == "continuous" and traits.group_count >= 2 and traits.n_samples >= 6:
        return AnalysisRecommendation(
            "existing_data_reanalysis", "bounded_group_comparison", True,
            ("continuous_target", "multiple_groups", "minimum_sample_contract_met"),
        )
    if traits.n_numeric_predictors >= 2 and traits.n_samples >= 5:
        return AnalysisRecommendation(
            "existing_data_reanalysis", "exploratory_structure_analysis", True,
            ("multiple_numeric_variables", "exploratory_only"),
        )
    return AnalysisRecommendation(
        "review", "insufficient_analysis_preconditions", False,
        ("no_registered_analysis_contract_has_satisfied_preconditions",),
    )


__all__ = [
    "AnalysisRecommendation", "AnalysisTraits", "ComparabilityContext", "ComparabilityDecision",
    "CrossSourceReasoningError", "EffectEstimate", "SCHEMA_VERSION", "UncertaintyComponent",
    "assess_comparability", "combine_uncertainty", "detect_verified_directional_contradiction",
