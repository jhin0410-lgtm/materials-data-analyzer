...
def _typed_equal(observed: object, expected: object) -> bool:
    """Compare canonical JSON-like values without Python scalar aliases."""
...
def _authority_bearing(authority: Mapping[str, Any]) -> bool:
    return any(
...
def _validate_expectations(
    *,
    ...
    if expected_subject is not None:
        _require(
            _typed_equal(subject_identities, expected_subject),
            "material/subject identity substitution detected",
        )
    expected_bindings = expected_map.get("source_bindings")
    if expected_bindings is not None:
        _require(
            _typed_equal(normalized_bindings, expected_bindings),
            "source role/path/artifact substitution detected",
        )
    expected_units = expected_map.get("result_units")
    if expected_units is not None:
        _require(
            _typed_equal(dict(result_units), expected_units),
            "result unit drift detected",
        )
    expected_calibration = expected_map.get("calibration_status")
    if expected_calibration is not None:
        _require(
            _typed_equal(calibration_status, expected_calibration),
            "calibration status promotion/drift detected",
        )
    expected_uncertainty = expected_map.get("uncertainty_status_by_id")
    if expected_uncertainty is not None:
        _require(
            _typed_equal(dict(uncertainty_statuses), expected_uncertainty),
            "uncertainty status promotion/drift detected",
        )
    ...
    expected_packet_sha = expected_map.get("packet_sha256")
    if expected_packet_sha is not None:
        expected_digest = _sha(expected_packet_sha, field="validation expectations.packet_sha256")
        _require(packet.get("packet_sha256") == expected_digest, "authenticated packet SHA-256 expectation drifted")
...
def _validate_evidence_packet(
    ...
    *,
    artifacts: Mapping[str, bytes] | None,
    expected: Mapping[str, Any] | None,
    require_external_authority_authentication: bool,
) -> dict[str, Any]:
    ...
    authority = _validate_authority(packet.get("authority"), evidence_kind=evidence_kind, validity=validity)

    if require_external_authority_authentication and _authority_bearing(authority):
        _require(
            artifacts is not None,
            "authority-bearing EvidencePacket requires exact artifact bytes",
        )
        _require(
            expected is not None,
            "authority-bearing EvidencePacket requires authenticated external expectations",
        )
        expected_map = _exact_keys(expected, _EXPECTATION_KEYS, field="validation expectations")
        expected_packet_sha = expected_map.get("packet_sha256")
        _require(
            expected_packet_sha is not None,
            "authority-bearing EvidencePacket requires authenticated packet SHA-256 expectation",
        )
        _sha(expected_packet_sha, field="validation expectations.packet_sha256")
...
def validate_evidence_packet(
    ...
    *,
    ...
    """Validate one packet; positive authority requires trust anchors outside the packet."""

    return _validate_evidence_packet(
        value,
        artifacts=artifacts,
        expected=expected,
        require_external_authority_authentication=True,
    )


def finalize_evidence_packet(unsigned_packet: Mapping[str, Any]) -> dict[str, Any]:
    """Add the self-hash after structural validation; this does not authenticate authority."""
    ...
    return _validate_evidence_packet(
        ...
        artifacts=None,
        expected=None,
        require_external_authority_authentication=False,
    )
