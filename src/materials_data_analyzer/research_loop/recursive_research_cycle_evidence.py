...
consumes an exact canonical verification record from those layers, binds the resulting
evaluated graph and persistent hypothesis portfolio, and decides whether the recursive
cycle must re-diagnose or stop.
"""
...
def _checkpoint(value: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    if value.get("schema_version") != RECURSIVE_CYCLE_SCHEMA_VERSION:
        raise RecursiveResearchEvidenceError("unsupported recursive checkpoint schema_version")
    if value.get("policy_version") != RECURSIVE_CYCLE_POLICY_VERSION:
        raise RecursiveResearchEvidenceError("unsupported recursive checkpoint policy_version")
...
def _execution_record(
    value: Mapping[str, Any], *, checkpoint_sha: str
) -> tuple[str, dict[str, Any]]:
    if value.get("schema_version") != VERIFIED_EXECUTION_RECORD_SCHEMA_VERSION:
        raise RecursiveResearchEvidenceError("unsupported verified execution record schema_version")
    ...
    if value.get("authorization_status") != "explicit_request_authorized_by_existing_chain":
        raise RecursiveResearchEvidenceError(
    ...
    if not isinstance(success, bool):
        raise RecursiveResearchEvidenceError("verified_execution_record.execution_success must be boolean")
...
def _graph_target(graph: Mapping[str, Any], target: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    node = matches[0]
    if node.get("node_type") != target["node_type"] or node.get("statement") != target["statement"]:
        raise RecursiveResearchEvidenceError("evaluated graph target identity was substituted")
...
def _transition_record(
    ...
    *,
    ...
    if value.get("target_node_id") != target["node_id"]:
        raise RecursiveResearchEvidenceError("epistemic transition target substitution detected")
    _text(value.get("transition_id"), "epistemic_transition_record.transition_id")
    _sha(value.get("consumer_verification_sha256"), "epistemic_transition_record.consumer_verification_sha256")
    if value.get("consumer_verification_status") != "verified_by_authenticated_transition_consumer":
        raise RecursiveResearchEvidenceError(
...
def _portfolio(
    ...
    *,
    ...
    digest = _embedded_sha(value, field="hypothesis_portfolio", sha_field="portfolio_sha256")
    if value.get("graph_id") != graph.get("graph_id"):
        raise RecursiveResearchEvidenceError("hypothesis portfolio graph_id drifted")
    binding = _mapping(value.get("evaluated_graph_binding"), "hypothesis_portfolio.evaluated_graph_binding")
    ...
    if record.get("statement") != target["statement"]:
        raise RecursiveResearchEvidenceError("portfolio hypothesis statement drifted")
    if record.get("epistemic_status") != assessment.get("status"):
        raise RecursiveResearchEvidenceError(
            "portfolio epistemic status does not match evaluated graph assessment"
        )
    state = _text(record.get("portfolio_state"), "hypothesis_portfolio target portfolio_state")
    return digest, {
        "hypothesis_id": target["node_id"],
        "epistemic_status": record["epistemic_status"],
        "portfolio_state": state,
        "research_directive": _text(
...
def advance_recursive_cycle_after_verified_transition(
    *,
    ...
    checkpoint_sha, target = _checkpoint(_mapping(authorization_checkpoint, "authorization_checkpoint"))
