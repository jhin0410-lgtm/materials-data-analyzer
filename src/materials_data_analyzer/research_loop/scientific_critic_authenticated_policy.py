...
def _consumer_graph_binding(consumer: Mapping[str, Any]) -> tuple[str, str]:
    binding = consumer.get("graph_binding")
    if not isinstance(binding, Mapping):
        raise ScientificCriticError("authenticated transition consumer graph_binding is malformed")
    path = binding.get("path")
    digest = binding.get("sha256")
    if path != "epistemic_graph.json":
        raise ScientificCriticError(
            "authenticated critic adapter requires the fixed epistemic_graph.json consumer binding"
        )
    if (
        ...
        raise ScientificCriticError(
            "authenticated transition consumer graph_binding.sha256 must be canonical lowercase SHA-256"
        )
    return path, digest
...
def build_authenticated_scientific_critic_report(
    ...
    *,
    ...
    graph_relative_path, expected_graph_sha256 = _consumer_graph_binding(consumer)
    graph_path = root / graph_relative_path
    if _sha256_file(graph_path) != expected_graph_sha256:
        raise ScientificCriticError(
            "epistemic graph changed after independent bundle authentication and before critic evaluation"
        )
    ...
    if _sha256_file(graph_path) != expected_graph_sha256:
        raise ScientificCriticError(
            "epistemic graph changed while the base critic report was being evaluated"
        )
