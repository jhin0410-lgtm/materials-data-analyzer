...
def _context_float(context: Mapping[str, Any], field: str, default: float) -> float:
    ...
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise ValidatedRecursivePlanningError(f"previous validation_inputs.{field} must be numeric")
    return float(value)


def _reconstruct_predecessor_context(context: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    ctx = _mapping(context, "previous_validated_planning_context")
    artifact = _mapping(ctx.get("validated_planning_artifact"), "previous_validated_planning_context.validated_planning_artifact")
    inputs = _mapping(ctx.get("validation_inputs"), "previous_validated_planning_context.validation_inputs")
    verification = validate_validated_recursive_planning_checkpoint(
        artifact,
        planning_handoff=_mapping(inputs.get("planning_handoff"), "previous validation_inputs.planning_handoff"),
        source_discrepancy_report=_mapping(inputs.get("source_discrepancy_report"), "previous validation_inputs.source_discrepancy_report"),
        source_evaluated_graph=_mapping(inputs.get("source_evaluated_graph"), "previous validation_inputs.source_evaluated_graph"),
        fresh_plan=_mapping(inputs.get("fresh_plan"), "previous validation_inputs.fresh_plan"),
        planner_program_state=_mapping(inputs.get("planner_program_state"), "previous validation_inputs.planner_program_state"),
        source_hypothesis_portfolio=_optional_mapping(inputs.get("source_hypothesis_portfolio"), "previous validation_inputs.source_hypothesis_portfolio"),
        previous_discrepancy_report=_optional_mapping(inputs.get("previous_discrepancy_report"), "previous validation_inputs.previous_discrepancy_report"),
        candidate_match=_optional_mapping(inputs.get("candidate_match"), "previous validation_inputs.candidate_match"),
        planner_critic_report=_optional_mapping(inputs.get("planner_critic_report"), "previous validation_inputs.planner_critic_report"),
        planner_reasoning_proposal=_optional_mapping(inputs.get("planner_reasoning_proposal"), "previous validation_inputs.planner_reasoning_proposal"),
        budget_units=_context_float(inputs, "budget_units", 8.0),
        minimum_utility=_context_float(inputs, "minimum_utility", 0.01),
        previous_validated_planning_context=_optional_mapping(inputs.get("previous_validated_planning_context"), "previous validation_inputs.previous_validated_planning_context"),
    )
    checkpoint = _mapping(verification.get("recursive_checkpoint"), "previous validated planning recursive_checkpoint")
    digest = verification.get("validated_checkpoint_sha256")
    if not isinstance(digest, str) or len(digest) != 64:
        raise ValidatedRecursivePlanningError("previous validated planning artifact omitted canonical SHA-256")
...
def build_validated_recursive_planning_checkpoint(
    *,
    ...
    try:
        ...
        raise ValidatedRecursivePlanningError("planning handoff failed hardened discrepancy-source reconstruction") from exc
    ...
    if previous_validated_planning_context is not None:
        reconstructed_previous, previous_artifact_sha = _reconstruct_predecessor_context(previous_validated_planning_context)
        if previous_checkpoint is not None and dict(previous_checkpoint) != reconstructed_previous:
            raise ValidatedRecursivePlanningError("raw previous checkpoint differs from reconstructed predecessor")
    elif previous_checkpoint is not None:
        raise ValidatedRecursivePlanningError("raw predecessor checkpoint is not accepted without deterministic validated-planning reconstruction")
    ...
    if planner_verification["plan_sha256"] != checkpoint["ancestry"]["fresh_plan_sha256"]:
        raise ValidatedRecursivePlanningError("verified planner SHA diverged before recursive checkpoint publication")
    if handoff_verification["handoff_sha256"] != checkpoint["ancestry"]["planning_handoff_sha256"]:
        raise ValidatedRecursivePlanningError("verified discrepancy handoff SHA diverged before checkpoint publication")
    result: dict[str, Any] = {
        ...
        "predecessor_validation": None if reconstructed_previous is None else {
            "validated_planning_artifact_sha256": previous_artifact_sha,
            "recursive_checkpoint_sha256": reconstructed_previous["checkpoint_sha256"],
...
def validate_validated_recursive_planning_checkpoint(
    ...
    *,
    ...
    if not isinstance(embedded, str) or len(embedded) != 64:
        raise ValidatedRecursivePlanningError("validated planning artifact SHA-256 is malformed")
    unsigned = dict(supplied)
    unsigned.pop("validated_checkpoint_sha256", None)
    if _canonical_sha256(unsigned) != embedded:
        raise ValidatedRecursivePlanningError("validated planning artifact SHA-256 does not match canonical content")
    ...
    if rebuilt != supplied:
        raise ValidatedRecursivePlanningError("validated planning artifact differs from deterministic reconstruction")
    checkpoint = _mapping(rebuilt.get("recursive_checkpoint"), "validated planning artifact recursive_checkpoint")
    return {
