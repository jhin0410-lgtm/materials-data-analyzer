...
headers, filenames, rows, or repeated values into accepted materials semantics or physical
lineage. Exact proposal hashes can be reviewed later through the existing human-review
release contract.
"""
...
def _text(value: object, field: str) -> str:
    if not isinstance(value, str) or not value.strip() or value != value.strip():
        raise GenericSemanticLineageProposalError(f"{field} must be non-empty trimmed text")
    return value
...
def _sha(value: object, field: str) -> str:
    ...
    if not _SHA_RE.fullmatch(text):
        raise GenericSemanticLineageProposalError(f"{field} must be lowercase SHA-256")
    return text


def _validate_generic_structure(structure: Mapping[str, Any]) -> str:
    artifact_sha = _sha(structure.get("artifact_sha256"), "structure.artifact_sha256")
    required_false = {
        "accepted_for_analysis": structure.get("accepted_for_analysis"),
        "measurement_semantics_interpreted": structure.get("measurement_semantics_interpreted"),
        "units_interpreted": structure.get("units_interpreted"),
        "sample_identity_inferred": structure.get("sample_identity_inferred"),
        "replicate_independence_inferred": structure.get("replicate_independence_inferred"),
        "calibration_semantics_interpreted": structure.get("calibration_semantics_interpreted"),
        "scientific_support_established": structure.get("scientific_support_established"),
        "scientific_status_changed": structure.get("scientific_status_changed"),
    }
    invalid = sorted(key for key, value in required_false.items() if value is not False)
    if invalid:
        raise GenericSemanticLineageProposalError(
    ...
    if not isinstance(profiles, list):
        raise GenericSemanticLineageProposalError("generic structure column_profiles must be a list")
    return artifact_sha
...
def _column_candidates(structure: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for profile in profiles:
        if not isinstance(profile, Mapping):
            raise GenericSemanticLineageProposalError("column profile must be an object")
        ...
        if isinstance(index, bool) or not isinstance(index, int) or index < 0:
            raise GenericSemanticLineageProposalError("column_index must be a non-negative integer")
        if not isinstance(header, str):
            raise GenericSemanticLineageProposalError("header_candidate must be text")
        if not isinstance(hints, list) or any(
            not isinstance(item, str) or item not in _ALLOWED_HINTS for item in hints
        ):
            raise GenericSemanticLineageProposalError("column semantic hints are invalid")
        result.append(
            {
                "column_index": index,
                ...
                "constant_nonblank_signal": profile.get("constant_nonblank_signal"),
...
def build_generic_semantic_lineage_proposal(
    *,
    ...
    semantic_proposal: dict[str, Any] = {
        ...
        "candidate_temperature_columns": _hint_columns(columns, "temperature_like"),
        "candidate_measurement_columns": _hint_columns(columns, "measurement_like"),
        "unresolved_normalized_measurement_fields": list(_REQUIRED_MEASUREMENT_FIELDS),
    ...
    lineage_proposal: dict[str, Any] = {
        ...
        "identity_like_source_columns_proposal_only": _hint_columns(columns, "identity_like"),
        "replicate_like_source_columns_proposal_only": _hint_columns(columns, "replicate_like"),
...
def verify_generic_semantic_lineage_proposal(
    *,
    ...
    """Recompute all proposal/review bindings and fail closed on mutated bytes."""
    if not isinstance(proposal, Mapping):
        raise GenericSemanticLineageProposalError("proposal must be an object")
    candidate = _text(proposal.get("candidate_id"), "proposal.candidate_id")
    ...
    try:
        ...
        raise GenericSemanticLineageProposalError("proposal review request is invalid") from exc
