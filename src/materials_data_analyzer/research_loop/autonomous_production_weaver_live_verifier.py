...
A successful Weaver artifact is accepted only after its retained source packet is authenticated
and replayed through the Weaver acquisition parser without network access.  The persisted evidence,
source bytes, cycle chain, capability verification, and unchanged scientific gates must all agree.
"""
...
def _verify_cycle_chain(cycles: object) -> list[dict[str, Any]]:
    _require(
        isinstance(cycles, list) and len(cycles) == 14,
        "Weaver cycle history must contain 14 cycles",
    )
...
def _verify_weaver_success(root: Path, manifest: dict[str, Any], stop: dict[str, Any]) -> str:
    ...
    _require(manifest.get("schema_version") == "2.0", "Weaver manifest schema drifted")
    _require(manifest.get("policy_version") == "2.0", "Weaver manifest policy drifted")
    ...
    specification = _load(root, "capability-specification-5.json")
    specification_sha = _verify_self_hash(
        specification,
        "capability_specification_sha256_without_self_field",
        label="Weaver capability specification",
    )
    ...
    _require(
        candidate.get("scientific_status_change_authorized") is False,
        "Weaver candidate gained scientific authority",
    )
    ...
    _require(
        verification.get("historical_reverification_network_requests_performed") == 0,
        "Weaver verification widened historical network authority",
    )

    smoke = verification.get("real_source_smoke_receipt")
    _require(isinstance(smoke, Mapping), "Weaver smoke receipt is missing")
    smoke_dict = dict(smoke)
    _verify_self_hash(
        smoke_dict,
        "report_sha256_without_self_field",
        label="Weaver smoke receipt",
    )
    _require(smoke.get("live_network_requests_performed") == 1, "Weaver live verifier request count drifted")
    _require(
        smoke.get("deterministic_replay_fetch_invocations") == 1
        and smoke.get("deterministic_replay_network_requests_performed") == 0,
        "Weaver deterministic replay contract drifted",
    )
    _require(
        smoke.get("execution_must_replay_retained_bytes") is True,
        "Weaver execution replay requirement disappeared",
    )
    ...
    _require(
        qualification.get("authority_extension_id") == AUTHORITY_EXTENSION_ID,
        "Weaver authority extension drifted",
    )
    ...
    _require(
        authorization.get("pmcid_derived_from_separately_pinned_policy") is True,
        "Weaver PMCID was not policy-pinned",
    )
    ...
    _require(
        authorization.get("scientific_status_change_authorized") is False,
        "Weaver authorization promoted science",
    )
    ...
    _require(
        evidence.get("acquisition_status")
        == "exact_weaver_primary_full_text_acquired_and_identity_verified",
        "Weaver acquisition status drifted",
    )
    ...
    _require(
        evidence.get("literature_promoted_to_row_level_measurement_authority") is False,
        "literature gained row authority",
    )
    _require(
        evidence.get("acquisition_success_establishes_scientific_bridge") is False,
        "acquisition self-promoted bridge",
    )
    ...
    _require(isinstance(replay_evidence, Mapping), "Weaver replay evidence is missing")
    replay_fetcher, retained_context = smoke_replay.authenticate_smoke_replay_evidence(
        replay_evidence,
        action_class=WEAVER_ACTION_CLASS,
    ...
    _require(isinstance(retained_context, Mapping), "Weaver replay context is missing")
    replayed = acquisition.execute_derived_weaver_acquisition(
        authorization=authorization,
        fetcher=replay_fetcher,
    )
    replay_fetcher.assert_consumed()
    replayed_sha = _verify_self_hash(
        replayed,
        "report_sha256_without_self_field",
        label="zero-network replayed Weaver evidence",
    )
    replayed_source = replayed.get("source")
    _require(isinstance(replayed_source, Mapping), "replayed Weaver source binding is missing")
    _require(replayed_sha == evidence_sha, "persisted Weaver evidence differs from retained-byte replay")
    _require(
        replayed_source.get("source_sha256") == source_sha,
        "persisted Weaver source differs from retained-byte replay",
    )
    _require(smoke.get("weaver_evidence_sha256") == evidence_sha, "smoke/evidence SHA binding drifted")
    _require(smoke.get("weaver_source_sha256") == source_sha, "smoke/source SHA binding drifted")

    identity = evidence.get("article_identity")
    _require(
        isinstance(identity, Mapping) and identity.get("article_identity_established") is True,
        "Weaver article identity is not established",
    )
    ...
    for claim_id in (
        ...
        _require(
            claims.get(claim_id, {}).get("matched") is True,
            f"required Weaver claim failed: {claim_id}",
        )
    ...
    _require(
        "50" not in ammt_fragments and "256" not in ammt_fragments,
        "Naderi/mds2 numeric spot range leaked into Weaver authority",
    )
    _require(
        claims.get("weaver-explicit-mds2-id", {}).get("matched") is False,
        "Weaver unexpectedly established explicit mds2 id",
    )
    _require(
        claims.get("weaver-explicit-power-conversion", {}).get("matched") is False,
        "Weaver unexpectedly established power conversion",
    )
    ...
    _require(
        gate.get("machine_setting_to_calibrated_power_relation_established") is False,
        "power calibration was transferred",
    )
    ...
    _require(
        gate.get("direct_numerical_cross_source_validation_authorized") is False,
        "direct numerical validation was authorized",
    )
    ...
    _require(
        cycle14.get("capability_verification_sha256") == verification_sha,
        "cycle 14 verification binding drifted",
    )
    _require(cycle14.get("verifier_live_network_requests_performed") == 1, "cycle 14 live request count drifted")
    _require(cycle14.get("execution_network_requests_performed") == 0, "cycle 14 execution used network")
    _require(
        cycle14.get("execution_source_bytes_from_authenticated_replay") is True,
        "cycle 14 did not use authenticated replay bytes",
    )
    ...
    _require(manifest.get("weaver_authorization_sha256") == authorization_sha, "manifest/authorization binding drifted")
    _require(manifest.get("weaver_full_text_acquisition_sha256") == evidence_sha, "manifest/evidence binding drifted")
    _require(manifest.get("weaver_source_sha256") == source_sha, "manifest/source binding drifted")
    _require(
        manifest.get("weaver_replay_evidence_sha256")
        == verification.get("real_source_smoke_replay_evidence_sha256"),
        "manifest/replay-evidence binding drifted",
    )
    _require(manifest.get("weaver_verifier_live_network_requests_performed") == 1, "manifest live request count drifted")
    _require(manifest.get("weaver_execution_network_requests_performed") == 0, "manifest execution used network")
    ...
    _require(
        manifest.get("exact_machine_setting_to_calibrated_power_relation_established") is False,
        "manifest promoted calibration transfer",
    )
    ...
    _require(
        manifest.get("direct_numerical_cross_source_validation_authorized") is False,
        "manifest authorized numerical validation",
    )
    ...
    _require(
        ...
        "manifest claimed scientific closeout",
    )
    ...
    return "weaver_14_cycle_success_verified_zero_network_replay"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Verify Weaver-capable autonomous production output."
    )
