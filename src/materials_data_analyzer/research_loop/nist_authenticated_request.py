...
from .action_authorization import AUTHORIZATION_POLICY_VERSION, assess_current_action_authorization
from .action_registry import describe_action, load_action_registry
from .authorized_execution import EXECUTION_POLICY_VERSION
...
def _reject_pairs(pairs):
    ...
    for key, value in pairs:
        if key in out:
            raise NistAuthenticatedRequestError(f"duplicate JSON key is not allowed: {key}")
        out[key] = value
    return out
...
def _json_bytes(raw: bytes, field: str) -> dict[str, Any]:
    try:
        ...
        raise NistAuthenticatedRequestError(f"{field} must be valid UTF-8 JSON") from exc
...
def _authenticate(
    *,
    ...
    if mission_sha != _sha(expected_mission_sha256, "expected_mission_sha256"):
        raise NistAuthenticatedRequestError("mission bytes do not match supplied trust root")
    ...
    if len(matches) != 1 or matches[0].get("max_cost_units") != ACTION_COST_UNITS:
        raise NistAuthenticatedRequestError("delegation policy does not bind exact NIST action")
...
def _current_selection(root: Path, run: Path, registry_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if authorization.get("authorization_status") != "ready_for_explicit_execution_request":
        raise NistAuthenticatedRequestError("current NIST state is not request-authorizable")
    ...
    if not isinstance(selected, Mapping) or not isinstance(contract, Mapping):
        raise NistAuthenticatedRequestError("NIST authorization omitted action contract")
    expected = {
    ...
    for key, value in expected.items():
        if selected.get(key) != value:
            raise NistAuthenticatedRequestError(f"NIST selected action drifted on {key}")
    if (
        ...
        raise NistAuthenticatedRequestError("NIST authorization execution contract drifted")
...
def compile_nist_authenticated_request(
    *,
    ...
    policy = _file(request_delegation_policy_path, "request_delegation_policy_path")
    ...
    if spec != expected_spec:
        raise NistAuthenticatedRequestError("only the frozen NIST Stage 1 spec may be bound")
    ...
    if contract.get("version") != ACTION_VERSION or contract.get("cost_units") != ACTION_COST_UNITS:
    ...
    if any(item.get("action_id") == action_id for item in state["actions"]):
        raise NistAuthenticatedRequestError("deterministic NIST action_id already exists")
    request = {
    ...
    manifest = {
        ...
        "policy_binding": {"path": str(policy), "policy_id": policy_id, "sha256": policy_sha},
        "registry_binding": {
        ...
        "research_binding": {"research_run": str(run), "ledger_sha256": ledger_sha},
        "selected_action_binding": {"sha256": selected_sha},
        "simulation_spec_binding": {"path": str(spec), "sha256": spec_sha},
        "request_binding": {
    ...
    try:
        ...
        (staging / "authenticated_request_manifest.json").write_bytes(_canonical_bytes(manifest))
...
def verify_nist_authenticated_request(
    *,
    ...
    expected_keys = {
        "schema_version", "action_id", "action_type", "research_run", "simulation_spec",
        "expected_simulation_spec_sha256", "registry", "repository_root",
        "expected_registry_sha256",
    }
    if set(request) != expected_keys:
        raise NistAuthenticatedRequestError("NIST request field set drifted")
    if request["schema_version"] != REQUEST_SCHEMA_VERSION or request["action_type"] != ACTION_TYPE:
    ...
    if spec != (root / EXPECTED_SPEC_RELATIVE_PATH).resolve(strict=True):
        raise NistAuthenticatedRequestError("NIST request simulation spec path drifted")
    spec_sha = _sha_file(spec)
    if request["expected_simulation_spec_sha256"] != spec_sha:
        raise NistAuthenticatedRequestError("NIST request simulation spec SHA drifted")
    registry = load_action_registry(registry_path, repository_root=root)
    ...
    if request["action_id"] != expected_id:
        raise NistAuthenticatedRequestError("NIST request deterministic action_id drifted")
    if any(item.get("action_id") == expected_id for item in state["actions"]):
        raise NistAuthenticatedRequestError("NIST request action_id already exists in ledger")
    ...
    if manifest.get("adapter_id") != ADAPTER_ID or manifest.get("action_type") != ACTION_TYPE:
    ...
    if manifest.get("mission_binding") != {"path": str(mission), "sha256": mission_sha}:
        raise NistAuthenticatedRequestError("NIST manifest mission binding drifted")
    if manifest.get("policy_binding") != {"path": str(policy), "policy_id": policy_id, "sha256": policy_sha}:
    ...
    if manifest.get("simulation_spec_binding") != {"path": str(spec), "sha256": spec_sha}:
        raise NistAuthenticatedRequestError("NIST manifest simulation spec binding drifted")
    ...
    if boundary != expected_boundary:
        raise NistAuthenticatedRequestError("NIST manifest widened authority boundary")
    ...
    if downstream != {
        ...
        raise NistAuthenticatedRequestError("NIST manifest downstream contract drifted")
    if manifest.get("selected_action_binding") != {"sha256": selected_sha}:
        raise NistAuthenticatedRequestError("NIST selected-action manifest binding drifted")
    ...
    if registry_binding != expected_registry_binding:
        raise NistAuthenticatedRequestError("NIST manifest registry binding drifted")
    if manifest.get("research_binding") != {"research_run": str(run), "ledger_sha256": ledger_sha}:
        raise NistAuthenticatedRequestError("NIST manifest research binding drifted")
    return {
        "schema_version": "1.0",
        "verifier_policy_version": VERIFIER_POLICY_VERSION,
        "verification_status": "bounded_nist_request_verified_eligible_for_existing_typed_executor",
        "adapter_id": ADAPTER_ID,
        "action_type": ACTION_TYPE,
        "action_version": ACTION_VERSION,
        "cost_units": ACTION_COST_UNITS,
