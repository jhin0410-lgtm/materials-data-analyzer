"""Fail-closed downstream-use eligibility for characterization handoff bundles.

This consumer-side implementation intentionally revalidates producer policy
rather than trusting the producer's declaration.  A passed gate authorizes only
the requested workflow category; it does not establish scientific validity.
"""
...
def _string_list(policy: Mapping[str, object], field: str) -> tuple[str, ...]:
    ...
    for item in value:
        ...
        if text in normalized:
            raise CharacterizationUsePolicyError(f"{field} must not contain duplicates")
...
def _validate_policy(
    ...
    *,
    ...
    if evidence_level in {"Inconclusive", "Unsupported"} and maximum_rank > _rank(
        "descriptive"
    ):
        raise CharacterizationUsePolicyError(
            f"{evidence_level} evidence cannot authorize use above descriptive"
        )
    if evidence_level == "Diagnostic" and maximum_rank > _rank("association"):
        raise CharacterizationUsePolicyError(
    ...
    if (
        ...
        and maximum_rank > _rank("descriptive")
    ):
        raise CharacterizationUsePolicyError(
            "unreviewed interpreted features cannot authorize use above descriptive"
        )
    if maximum_rank >= _rank("predictive"):
        if independence_group_field is None:
            raise CharacterizationUsePolicyError(
                "predictive or stronger use requires independence_group_field"
            )
        if measurement_timing != "pre_outcome":
            raise CharacterizationUsePolicyError(
                "predictive or stronger use requires pre_outcome measurement_timing"
            )
    if maximum_rank >= _rank("causal") and not causal_validated:
        raise CharacterizationUsePolicyError(
            "causal or stronger use requires causal_design_validated=true"
        )
    if maximum_rank >= _rank("engineering") and not operational_validated:
        raise CharacterizationUsePolicyError(
...
def _manifest_object(manifest_path: str | Path) -> dict[str, Any]:
    path = Path(manifest_path)
    if not path.is_file() or path.is_symlink():
        raise FileNotFoundError(f"characterization bundle manifest not found: {path}")
...
def evaluate_characterization_use(
    ...
    *,
    ...
    """Evaluate one requested use before any consumer outputs are generated."""
    ...
    if requested_rank >= _rank("predictive"):
        if split_group_field is None or not split_group_field.strip():
            reasons.append(
                "predictive or stronger use requires --split-group-field"
            )
        ...
        if not isinstance(context_columns, list) or independence_group_field not in context_columns:
            reasons.append(
...
def require_characterization_use(
    ...
    *,
    ...
    if not decision.allowed:
        raise CharacterizationUsePolicyError(
            "characterization downstream use blocked: " + "; ".join(decision.reasons)
        )
    return decision
