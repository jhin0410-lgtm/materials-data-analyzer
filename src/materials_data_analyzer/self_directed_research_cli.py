"""CLI for finite self-directed research planning iterations."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m materials_data_analyzer.self_directed_research_cli",
        description=(
            "Build one finite self-directed research iteration from a verified mission/program. "
            "The planner may synthesize evidence-search, analysis, simulation-design, and physical-"
            "experiment-design proposals, but never executes them or changes scientific status."
        ),
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="New immutable plan path. Existing files are rejected.",
    )
    return parser
