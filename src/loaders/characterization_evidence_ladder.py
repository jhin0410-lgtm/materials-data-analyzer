"""Independent verifier for characterization L0-L8 evidence-ladder handoffs.

This module intentionally does not import ``materials-characterization-analyzer``.
The consumer reconstructs the producer's canonical assessment from the embedded
normalized declaration and fails closed on any artifact or manifest-summary drift.
The ladder describes evidence maturity only; it is not empirical evidence and it
never authorizes downstream scientific use.
"""
...
class CharacterizationEvidenceLadderError(ValueError):
    """Raised when a characterization evidence-ladder binding cannot be replayed."""
...
def _normalize_subject(value: object) -> dict[str, str]:
    subject = _exact_mapping(
        value, required=_REQUIRED_SUBJECT_FIELDS, field="subject"
    )
    return {
...
def evaluate_characterization_evidence_ladder(value: object) -> dict[str, Any]:
    """Reproduce the producer's deterministic L0-L8 assessment independently."""
    root = _exact_mapping(
        value,
    ...
    declaration = {
        ...
        "limitations": _text_list(
            root["limitations"], "limitations", allow_empty=True
        ),
    }
    highest_index = -1
    ...
    first_blocking = (
        LEVELS[highest_index + 1]
        if highest_index + 1 < len(LEVELS)
        else None
    )
    non_supported = [
        {
...
def _validated_assessment(path: Path) -> dict[str, Any]:
    ...
    replayed = evaluate_characterization_evidence_ladder(
        _raw_declaration_for_replay(declaration)
    )
    if payload != replayed:
        raise CharacterizationEvidenceLadderError(
...
def _expected_record(path: Path, assessment: Mapping[str, Any]) -> dict[str, Any]:
...
def validate_characterization_evidence_ladder_record(
    ...
    *,
    ...
    """Verify artifact identity, replay summary, and exact bundle binding."""
    root = Path(bundle_root).resolve()
    record = _exact_mapping(
        value, required=_RECORD_FIELDS, field="scientific_evidence_ladder"
    )
    ...
    expected_record = _expected_record(path, assessment)
    ...
    if record.get("lower_level_evidence_preserved") is not True:
        raise CharacterizationEvidenceLadderError(
