...
- Consolidated internal feature stages v2.5.1-v2.5.2 and v2.6.1-v2.6.14 into
  one additive public semantic release; a separate v2.5.0 release was not
  created.
- Added version-allowlisted compatibility adapters for tracked Materials v2.2.4
  and Battery v2.3.5 artifacts with deterministic raw-byte and canonical
  logical-content checks. Materials remains `compatible_with_restrictions`,
  Battery remains `partial`, and provenance portability remains `diagnostic`.
...
- Added bounded SNL LFP source recovery, local ZIP identity and safe-entry
  binding, source-to-entry nomenclature review, schema observation, cycle-regime
  review, and transition-artifact closeout without promoting candidate labels to
  exact command or measurement semantics.
...
- Added the checksum-bound v2.6.14 evidence-line closeout. All 13 v2.6 stage
  ...
  the final scientific closeout remains `inconclusive`.
...
- Added pinned DWCNT, RWGS XRD/EDS, four-carbon-material, and NIST AM-Bench
  cross-repository workflows while preserving blocked methods, unresolved
...
- Added the one-command representative NIST process-characterization workflow,
  ...
  plan. The three-condition design remains
  `not_ready_for_predictive_or_causal_modeling`; no response model or process
  optimum is produced.
- Added public release/citation governance and deterministic cross-repository
  release-readiness audits. No Git tag, GitHub Release, or package publication
  is created by the offline audits.
